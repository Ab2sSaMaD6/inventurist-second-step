...
export default function useTable(records, headCells, filterFunction, onSelectAllClick, numSelected) {
    ...
    const recordsAfterPagingAndSorting = () => {
        return stableSort(filterFunction.fn(records), getComparator(order, orderBy)).slice(
            page * rowsPerPage,
            (page + 1) * rowsPerPage
        )
    }
    ...
    const TablePagination = () => (
        <MuiTablePagination
            ...
            labelDisplayedRows={() => {
                return <><span className='pageNumber'>{page+1}</span> of {Math.floor(rowCount/ rowsPerPage)+1} page </>
              }}
        />
    )
